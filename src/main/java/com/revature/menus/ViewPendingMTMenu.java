...
public class ViewPendingMTMenu extends AbstractMenu {
	...
	public List<MenuLine> buildMenu() {
		List<MoneyTransfer> mtList = bmts.viewAllPendingTransfers(MenuSelector.getMenuSelector().getCurrentUser().getUser_id());
		...
		List<MenuLine> lines = mtList.stream()
				.map(ele -> { 
					return new MenuLine(mtList.indexOf(ele),
										...
										()->{ 
											System.out.println(
													"Do you wish to cancel money transfer?\n" + 
													ele.display() + "\n (Y/N):");
											String input = this.getInputReader().nextLine();
											input = input.toUpperCase();
											if(input.equals("Y")||input.equals("YES")) {
												bmts.cancelTransfer(ele);
